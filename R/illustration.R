...
mystartupMessage <- function(..., domain=NULL, pkg="", type="version", SMHandler=mySMHandler){
 withRestarts(withCallingHandlers(startupMessage(..., domain=domain, pkg=pkg, type=type), 
...
if((!getOption("StartupBanner")=="off")||is.null(getOption("StartupBanner"))) 
       mystartupMessage(tit.vers$"title", " (version ", tit.vers$"ver", ")", domain=domain, 
                             pkg=pkg, type="version", SMHandler=SMHandler)
###
if((getOption("StartupBanner")=="complete")||is.null(getOption("StartupBanner"))) 
    {if (length(list(...))>0)
         mystartupMessage(..., domain=domain, pkg=pkg, type="notabene", SMHandler=SMHandler)
     ...
     if(!is.null(MANUAL))
        {if (all(substr(as.character(MANUAL),1,7)=="http://")) 
               {MANUALL <- TRUE
                MANUALS <- c("\n  ",MANUAL)}
         else  {MANUAL1 <- paste(MANUAL,sep=.Platform$file.sep,collapse=.Platform$file.sep)
                MANUALpath <- file.path(system.file(package=pkg),MANUAL1,collapse="")
                if (file.exists(MANUALpath)) {MANUALL<-TRUE
                                            MANUALS <- c("\n  ",MANUALpath)}  
                }
         }                          
     ...
     seps[2] <- ifelse(!is.null(NEWS)&&sum(!is.null(NEWS) , MANUALL, !is.null(URL))>1,gettext(", as well as",domain=domain),"")
     seps[3] <- ifelse(MANUALL&&sum(MANUALL, !is.null(URL))>1,", ","")
     if( (MANUALL|| !is.null(URL)) && is.null(NEWS)) seps[1] <- gettext(", as well as",domain=domain) 
     #
     if (L>0)
